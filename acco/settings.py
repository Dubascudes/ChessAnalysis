...
import sqlite3
from .fetch_games import fetch_month_pgn, parse_pgn_games, fetch_current_month_games_and_save_to_db
...
class SettingsDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        # Add the Update Games button
        self.btn_update_games = ttk.Button(username_frame, text="Update Games for Current User",
                                           command=self.update_user_games)
        self.btn_update_games.grid(row=0, column=2, padx=10, pady=5, sticky="ew")
        
        # Update button text if username changes in the entry field
        self.username_var.trace_add("write", self._update_button_text)
        ...
        # Stockfish settings
        # stockfish_frame = ttk.LabelFrame(main_frame, text="Stockfish Settings", padding="5")
        # username_frame.pack(fill="x", pady=5)
        
        ttk.Label(eval_frame, text="Stockfish Path:").grid(row=2, column=0, padx=0, pady=5, sticky="e")
        self.stockfish_path_var = tk.StringVar(value=self.settings.get('stockfish_path', STOCKFISH_PATH))
        self.stockfish_entry = ttk.Entry(eval_frame, textvariable=self.stockfish_path_var)
        self.stockfish_entry.grid(row=2, column=1, padx=5, pady=5, sticky="ew")
    ...
    def load_settings(self):
        ...
        self.settings = {}
        try:
            if os.path.exists('acco/resources/settings.json'):
                with open('acco/resources/settings.json', 'r') as f:
    ...
    def save_settings(self):
        ...
        try:
            ...
            with open('acco/resources/settings.json', 'w') as f:
                json.dump(settings, f, indent=4)
            
            # Update global variables
            global USER, DB_FILE
            USER = settings['username']
            DB_FILE = f"acco/resources/{USER.lower()}_games.db"
    ...
    def populate_database_list(self):
        ...
        # 1) locate the resources folder (assuming this file lives in acco/...)
        pkg_dir      = os.path.dirname(__file__)                   # .../path/to/acco
        resources_dir = os.path.join(pkg_dir, "resources")         # .../path/to/acco/resources
        
        # 2) glob inside resources_dir
        pattern = os.path.join(resources_dir, "*_games.db")
        db_paths = glob.glob(pattern)
        
        for db_path in db_paths:
            try:
                # 3) pull stats from the full path
                stats    = os.stat(db_path)
                size     = stats.st_size
                modified = stats.st_mtime
                
                # 4) pretty-print the size
                if size < 1024:
                    size_str = f"{size} B"
                elif size < 1024 * 1024:
                ...
                # 5) pretty-print the date
                date_str = datetime.fromtimestamp(modified).strftime('%Y-%m-%d %H:%M:%S')
                
                # 6) strip off the path so only the filename appears
                filename = os.path.basename(db_path)
                
                # 7) insert into treeview
                self.db_tree.insert("", "end", values=(filename, size_str, date_str))
    ...
    def create_new_database(self):
        ...
        try:
            ...
            # Create database
            db_file = f"acco/resources/{username.lower()}_games.db"
    ...
    def _update_button_text(self, *args):
        ...
        if current_username:
            self.btn_update_games.config(text=f"Update Games for '{current_username}'")
        else:
            self.btn_update_games.config(text="Update Games for Current User")

    def update_user_games(self):
        username_to_update = self.username_var.get().strip()
        if not username_to_update:
            messagebox.showwarning("Update Games", "Please enter a username first.", parent=self)
            return

        current_db_file = f"acco/resources/{username_to_update.lower().replace(' ', '_')}_games.db"

        # Disable button during update
        self.btn_update_games.config(state="disabled", text="Updating...")
        self.update_idletasks()

        try:
            print(f"Attempting to update games for {username_to_update} into {current_db_file}...")
            # This function should ideally run in a separate thread if it's long.
            # For now, direct call.
            success, message = fetch_current_month_games_and_save_to_db(username_to_update, current_db_file)

            if success:
                messagebox.showinfo("Update Games", f"Successfully updated games for {username_to_update}. \n{message}", parent=self)
                self.games_updated = True
                self.populate_database_list()
            else:
                messagebox.showerror("Update Games", f"Failed to update games for {username_to_update}. \n{message}", parent=self)
        except Exception as e:
            messagebox.showerror("Update Games", f"An error occurred: {e}", parent=self)
            print(f"Error during game update for {username_to_update}: {e}")
        finally:
            # Re-enable button and restore text
            self.btn_update_games.config(state="normal")
            self._update_button_text()
